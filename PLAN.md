...
> **v0.2.0 에서 일부 변경됨 — 회사 데이터만 Supabase 로 이동.**
> 아래 근거는 *계산* 에는 그대로 유효하다(계산은 지금도 100% 클라이언트). 바뀐 것은 데이터 공급뿐이다.
> 이유: 회사 지표가 저장소 안 JSON 이면 **지표 수정에 코드 변경과 배포가 필요해**, 디자인 작업과
> 데이터 작업이 같은 PR 에서 충돌한다. 지금은 `companies` 테이블이 소스이고 RootLayout(서버)이
> 60초 ISR 로 읽어 Context 로 내려준다. 닿지 않으면 저장소 JSON 으로 폴백하고 그 사실을 푸터에 표시한다.
> API 라우트는 여전히 0개다 — Server Component 가 직접 읽는다.
