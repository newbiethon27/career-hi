...
export default function DashboardPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-8 px-4 py-10 sm:py-14">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
            <span className="rounded-full bg-muted px-2 py-0.5 text-[11px]">부가 기능</span>
            <span>
          ...
          <h1 className="text-2xl font-bold tracking-tight sm:text-3xl">{company.name}와(과) 당신의 적합도</h1>
        </div>
        ...
        <ScoreHero
          ...
          sub={
            <>
              ...
              <span className="font-medium text-foreground">{1 + rec.ranked.filter((r) => r.fit.fit > current.fit.fit).length}위</span>
            </>
          }
        />
      ...
      <section className="rounded-2xl border-l-4 border-primary bg-primary/5 p-5 text-[15px] leading-relaxed">{current.diagnosis}</section>
