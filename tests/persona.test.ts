...
describe("데모 페르소나", () => {
  ...
  it("같은 회사·직군·연봉이어도 성향이 다르면 추천이 달라진다", () => {
    ...
    expect(rA.current.fit.fit).not.toBe(rB.current.fit.fit);
  });

  it("A 는 현재 회사 Fit 이 B 보다 낮고, 추천이 존재한다", () => {
    expect(rA.current.fit.fit).toBeLessThan(rB.current.fit.fit);
  ...
  it("A 는 이직 검토 밴드, B 는 검토 밴드가 아니며 지수가 A 보다 낮다", () => {
    const mtA = computeMoveTiming({
      currentFit: rA.current.fit.fit,
      bestFit: rA.top[0]?.fit.fit ?? null,
      tenureMonths: SHARED_PROFILE.tenureMonths,
      currentSalary: SHARED_PROFILE.currentSalary,
      companyAvgSalary: rA.current.company.metrics.avgSalaryManwon?.value ?? null,
    });
    const mtB = computeMoveTiming({
      currentFit: rB.current.fit.fit,
      bestFit: rB.top[0]?.fit.fit ?? null,
      tenureMonths: SHARED_PROFILE.tenureMonths,
      currentSalary: SHARED_PROFILE.currentSalary,
      companyAvgSalary: rB.current.company.metrics.avgSalaryManwon?.value ?? null,
    });
    // 데모 리허설용 출력 — 실제 값 확인
    console.log("[persona A]", rA.current.fit.fit, rA.top.map((t) => `${t.company.name}:${t.fit.fit}`), "MT", mtA.score, mtA.band);
    console.log("[persona B]", rB.current.fit.fit, rB.top.map((t) => `${t.company.name}:${t.fit.fit}`), "MT", mtB.score, mtB.band);
